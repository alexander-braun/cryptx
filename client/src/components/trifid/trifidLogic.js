const Trifid = (() => {
    ...
    const genKey = () => {
        const key = []
        for(let i = 0; i < 26; i++) {
            if(trifidKey[i] && key.indexOf(trifidKey[i].toLowerCase()) === -1 && alphabet.indexOf(trifidKey[i].toLowerCase()) !== -1) {
    ...
    const generateGroup = (groupNumber) => {
        ...
        let group = []
        for(let i = groupNumber * groupSize; i < groupNumber * groupSize + groupSize; i++) {
