const Rsa = (() => {
    ...
    const encrypt = () => {
        ...
        if (bigInt(gcd).compare(bigInt(1)) !== 0) return ['!!! φ(n) and e are not coprime - gcd of φ(n) and e is ' + gcd + ' Please check that you have two prime numbers and an appropriate e without a common gcd!!!', '']
    ...
    const calc = (direction) => {
        if(typeof calcD() === 'object') return ['!!! φ(n) and e are not coprime - gcd of φ(n) and e is ' + calcD().join('') + ' Please check that you have two prime numbers and an appropriate e without a common gcd!!!', '']
